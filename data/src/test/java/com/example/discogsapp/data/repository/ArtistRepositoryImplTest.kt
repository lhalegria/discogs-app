...
class ArtistRepositoryImplTest {
    ...
    @Test
    fun `searchArtists maps service response and forwards query params`() = runTest {
    ...
    @Test
    fun `getArtistDetails maps service response and forwards artist id`() = runTest {
        val service = mockk<DiscogsService>()
        coEvery {
            service.getArtistDetails(20)
        } returns ArtistDetailsResponse(
            id = 20,
            name = "Aphex Twin",
            profile = null,
            realName = null,
        ...
        assertEquals("Aphex Twin", result.name)
        assertEquals("", result.profile)
        assertEquals("", result.images.first().smallUri)
    }

    @Test
    fun `getArtistReleases maps service response and forwards query params`() = runTest {
        val service = mockk<DiscogsService>()
        coEvery {
        ...
        val result = repository.getArtistReleases(
